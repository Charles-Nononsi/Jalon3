...
drop user backend;
create user backend with password 'Guinssa0711';
create database backenddb with template=template0 owner=backend;
\connect backenddb;
alter default privileges grant all on tables to backend;
alter default privileges grant all on sequences to backend;
